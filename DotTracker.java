...
class Vector2D {
	...
	public float dist(Vector2D that) {
...
class MyUtil {
	public static float distFromPointToLine(int px, int py, int x1, int y1, int x2, int y2) {
...
public class DotTracker extends JFrame {
	...
	public DotTracker() throws Exception {
	...
	public void onTimer() {
		...
		if (fileOut == null) {
			try {
				fileOut = new PrintWriter(new FileWriter("p2Trimmed.txt"), true);
			} catch (Exception e) {
			}
		}
		fileOut.println("frame = " + frameIndex);
		for (int i = 0; i < trackers.size(); i++) {
			Tracker t = (Tracker)trackers.get(i);
			
			fileOut.println("\ttracker = " + i);
			fileOut.println("\t\tx = " + t.center.x);
			fileOut.println("\t\ty = " + t.center.y);
		}
	...
	public void onMouseReleased(int x, int y) {
		...
		if (x != -1 && y != -1) {
			...
			if (t != null && newConnector.a != t) {
				newConnector.b = t;
				addConnector(newConnector);
			}
			if (t == null) {
				trackers.add(t = new Tracker(x, y));
				newConnector.b = t;
				addConnector(newConnector);
			}
		}
	...
	public void onMouseDragged(int x, int y) {
		if (newConnector == null) {
			newConnector = new Connector(selectedTracker, null);
		}
		if (x == -1 && y == -1) {
			x = newConnector.bx;
			y = newConnector.by;
		} else {
			newConnector.setBPos(x, y);
		}
	...
	public void addConnector(Connector newC) {
		connectors.add(newC);
		
		for (int i = 0; i < trackers.size(); i++) {
			Tracker t = (Tracker)trackers.get(i);
			Vector tConnectors = new Vector();
			boolean usesNewC = false;
			for (int ci = 0; ci < connectors.size(); ci++) {
				Connector c = (Connector)connectors.get(ci);
				if (c.a == t || c.b == t) {
					if (c == newC)
						usesNewC = true;
					tConnectors.add(c);
			...
			if (usesNewC && tConnectors.size() == 2) {
				new AngleTracker(t, (Connector)tConnectors.get(0), (Connector)tConnectors.get(1));
			}
		}
	}
	...
	public void removeTracker(Tracker trackerToRemove) {
		for (Iterator i = connectors.iterator(); i.hasNext(); ) {
			...
			if (c.a == trackerToRemove || c.b == trackerToRemove) {
				i.remove();
			}
		}
	...
	public void removeConnector(Connector connectorToRemove) {
		for (Iterator i = connectors.iterator(); i.hasNext(); ) {
			Connector c = (Connector)i.next();
			
			if (c == connectorToRemove) {
				i.remove();
			}
		}
		for (Iterator i = angleTrackers.iterator(); i.hasNext(); ) {
			...
			if (a.usesConnector(connectorToRemove)) {
				i.remove();
			}
		}
	}
	...
	public Connector getConnector(int x, int y) {
		...
		for (int i = 0; i < connectors.size(); i++) {
			...
			float dist = MyUtil.distFromPointToLine(x, y, c.a.center.x, c.a.center.y, c.b.center.x, c.b.center.y);
			if (dist < closestDist) {
				closestDist = dist;
				closest = c;
			}
	...
	public AngleTracker getAngleTracker(Tracker t) {
		for (int i = 0; i < angleTrackers.size(); i++) {
			AngleTracker a = (AngleTracker)angleTrackers.get(i);
			if (a.center == t)
				return a;
	...
	class Connector {		
		...
		public Connector(Tracker a, Tracker b) {
			this.a = a;
			this.b = b;
		}
	...
	class AngleTracker {
		...
		public AngleTracker(Tracker center, Connector aCon, Connector bCon) {
			...
			a = aCon.a == center ? aCon.b : aCon.a;
			b = bCon.a == center ? bCon.b : bCon.a;
		...
		public boolean usesTracker(Tracker t) {
			return t == a || t == b || t == center;
		}
		public boolean usesConnector(Connector c) {
			return c == aCon || c == bCon;
		}
		public void update() {
			...
			Vector2D C = new Vector2D(center.center.x, center.center.y);
	...
	class Tracker {
		...
		public void draw2(Graphics2D g) {
			AngleTracker a = getAngleTracker(this);
			if (a != null)
				g.setColor(a.color);
			else		
				g.setColor(Color.GREEN);
